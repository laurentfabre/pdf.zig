...
| 10 | RTL text (Arabic, Hebrew) **+ vertical-writing CJK** | unicode bidi class; font `/WMode 1` | **v2 fix (RTL)**: emit as-is in source order, append `bidi-untreated` warning. Many PDF producers already store in logical order; full-string reversal breaks digits + punctuation per the Unicode bidi algorithm. Bidi-aware post-pass deferred to v1.x. **PR-15 [feat] (CJK vertical)**: vertical-writing CJK extraction is known-broken — XY-Cut++ assumes left-to-right horizontal flow and produces garbled column-order text on `/Identity-V` / `UniJIS-UCS2-V` / etc. The CLI now detects `/WMode 1` per page and emits a standalone `kind:"warning"` `vertical_writing_unsupported` NDJSON record (with the page still emitted on a best-effort basis). Real fix — a per-glyph rotation pass that re-projects vertical runs into reading order — is deferred. The synthetic CJK harness (`audit/cjk-pdfs/synthetic/`, `audit/v1_4_cjk_run.py`) gates 100 % warning emission on `wmode == 1` fixtures and ≥ 95 % char-identity on horizontal CJK. |
| 11 | CJK width handling | Unicode width category | Token estimator must use double-width counting |
| 12 | Vertical writing mode (`/V` Wmode) | font `/Wmode 1` | **PR-15 [feat]**: emit `kind:"warning"` `vertical_writing_unsupported` NDJSON record per affected page (see also case #10). Best-effort horizontal extraction continues; the warning lets downstream consumers (LLM ingestion, citation tools) flag the page for manual review. |
