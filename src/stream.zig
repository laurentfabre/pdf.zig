...
pub const Envelope = struct {
    ...
    pub fn emitInterrupted(self: *Envelope, signal_num: std.posix.SIG) !void {
        try self.beginRecord(.interrupted);
        try self.writer.print(",\"signal\":{d}", .{@intFromEnum(signal_num)});
        try self.endRecord();
    }
...
pub var interrupted_flag = std.atomic.Value(u32).init(0);

fn handleInterrupt(sig: std.posix.SIG) callconv(.c) void {
    interrupted_flag.store(@intFromEnum(sig), .seq_cst);
}
...
pub fn wasInterrupted() ?std.posix.SIG {
    const v = interrupted_flag.load(.seq_cst);
    return if (v == 0) null else @enumFromInt(v);
}
...
test "interrupted record carries signal number" {
    ...
    var aw = std.Io.Writer.fixed(&buf);
    var env = Envelope.initWithId(&aw, "x.pdf", FIXED_DOC_ID);
    try env.emitInterrupted(@enumFromInt(2));
    const written = aw.buffered();
